...
// Same All Members / Submitted / Pending drill-down as DistrictAreaDetails,
// scoped to the unit admin's own single unit instead of a district area.
const UnitAdminDetails = () => {
  ...
  const [section, setSection] = useState(() => {
    ...
    return ['all', 'submitted', 'pending'].includes(initial) ? initial : 'all'
  })
  ...
  }, [quarter, year, section, page, debouncedSearch])

  const fetchDetails = async () => {
    try {
      setListLoading(true)
      const response = await api.get('/unitadmin/details', {
        params: {
  ...
  const openSubmission = async (member) => {
    ...
    setFormSchema(null)
    try {
      const response = await api.get(`/unitadmin/submissions/${member.submission.submissionId}`)
    ...
    <div className="max-w-5xl mx-auto px-4 py-4 sm:py-6">
        ...
        <div className="min-w-0">
          ...
          <p className="ih-page-subtitle flex items-center gap-1">
            ...
            <span className="truncate">
              {data?.unit || '…'}{period ? ` · ${getQuarterName(period.quarter)} ${period.year}` : ''}
            </span>
          </p>
        </div>
      </div>
      ...
      {initialLoading ? (
        <div className="flex items-center justify-center py-16">
          <Loader2 className="w-6 h-6 text-[#7B4FF2] animate-spin mr-2" /> Loading unit details…
        </div>
      ) : !data ? (
        <p className="text-center text-gray-400 py-16">Unable to load unit details</p>
      ) : (
        <>
          {/* Stat cards */}
...
export default UnitAdminDetails
