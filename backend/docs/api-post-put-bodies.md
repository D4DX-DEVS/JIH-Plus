...
{
  "name": "Abdul Latheef",
  "nameEnglish": "Abdul Latheef",
  "fathersName": "Muhammed",
  "gender": "male",
  "ruknId": "KAR-2025-001",
  "district": "Pathanamthitta",
  "area": "Area",
  "localUnit": "Test Unit",
  "halkhaName": "Test Halkha",
  "address": "123 Main Street, Pathanamthitta",
  "mobile": "+91-9000000000",
  "compulsoryBooks": { "book1": true, "book2": false, "book3": false },
  "photo": "data:image/png;base64,..."
}
```
```json
{ "success": true, "message": "Karkun form submitted successfully. Waiting for unit admin verification.", "data": { "_id": "...", "status": "pending" } }
```

### PUT /:id/verify/unit
```json
{ "status": "approved", "comments": "Verified in person" }
```
```json
{ "success": true, "message": "Form approved by unit admin. Moved to area admin for review.", "data": { "_id": "...", "status": "unit_review" } }
```

### PUT /:id/verify/area
```json
{ "status": "approved", "comments": "All good" }
```
```json
{ "success": true, "message": "Form approved by area admin. Moved to district admin for review.", "data": { "_id": "...", "status": "area_review" } }
```

### PUT /:id/verify/district
```json
{ "status": "approved", "registrationNumber": "JIH-2025-0091" }
```
```json
{ "success": true, "message": "Form approved by district admin. Application fully approved!", "data": { "_id": "...", "status": "state_review" } }
```

### PUT /:id/verify/state
```json
{ "status": "approved", "comments": "Welcome" }
```
```json
{ "success": true, "message": "Form approved by state admin.", "data": { "_id": "...", "status": "approved" } }
```
...
### POST /submit
```json
{ "ruknId": "R-2025-001", "name": "Hamid Ali", "districtName": "Pathanamthitta", "areaName": "Area", "unitName": "Test Unit", "photo": "data:image/jpeg;base64,...", "jamaathMeeting": { "hadir": 5, "leave": 1, "absent": 0 } }
```
```json
{ "success": true, "message": "Rukn form submitted successfully. Waiting for unit admin verification.", "data": { "_id": "...", "status": "pending" } }
```

### PUT /:id/verify/unit
```json
{ "status": "approved", "comments": "Meets criteria", "opinion": "Strong candidate" }
```
```json
{ "success": true, "message": "Form approved by unit admin. Moved to district admin for review.", "data": { "_id": "...", "status": "district_review" } }
```

### PUT /:id/verify/district
```json
{ "status": "approved", "comments": "Recommend" }
```
```json
{ "success": true, "message": "Form approved by district admin. Moved to state admin for final review.", "data": { "_id": "...", "status": "state_review" } }
```

### PUT /:id/verify/state
```json
{ "status": "approved", "comments": "Approved" }
```
```json
{ "success": true, "message": "Form approved by state admin.", "data": { "_id": "...", "status": "approved" } }
```
