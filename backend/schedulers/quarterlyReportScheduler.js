 ...
 *
 * Runs: every day at midnight (Asia/Kolkata) so no instance is missed, once per
 * JIH Portal tenant (master + franchises), each against its own database.
 */
const runForCurrentTenant = async (tenantKey) => {
  const now = new Date();
  const tag = `[QuarterlyReportScheduler][${tenantKey}]`;
  console.log(`${tag} Running job at ${now.toISOString()}`);
  ...
  try {
    ...
    for (const template of templates) {
      ...
      console.log(
        `${tag} Created quarterly report ${newReport._id} ` +
        `from template ${templateRootId} for Q${periodQuarter} ${periodYear}`
      );
    }
  } catch (error) {
    console.error(`${tag} Error generating quarterly reports:`, error);
  }
};
  ...
  cron.schedule(
    ...
    async () => {
      for (const tenant of listTenants()) {
        if (!tenant.isReady()) {
          console.warn(`[QuarterlyReportScheduler][${tenant.key}] Skipped: database not connected`);
          continue;
        }
