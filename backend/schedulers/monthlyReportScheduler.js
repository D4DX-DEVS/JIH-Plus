 ...
 * Runs at midnight on the 1st of every month (Asia/Kolkata timezone), once per
 * JIH Portal tenant (master + franchises), each against its own database.
 */
const runForCurrentTenant = async (tenantKey) => {
  const now = new Date();
  const currentMonth = now.getMonth() + 1; // 1-12
  const currentYear = now.getFullYear();
  const tag = `[MonthlyReportScheduler][${tenantKey}]`;

  console.log(
    `${tag} Running job for ${currentYear}-${currentMonth
      .toString()
      .padStart(2, '0')}`
  );
  ...
  try {
    ...
    for (const template of templates) {
      ...
      console.log(
        `${tag} Created monthly report ${newReport._id} from template ${templateRootId} for ${currentYear}-${currentMonth}`
      );
    }
  } catch (error) {
    console.error(`${tag} Error generating monthly reports:`, error);
  }
};
  ...
  cron.schedule(
    ...
    async () => {
      for (const tenant of listTenants()) {
        if (!tenant.isReady()) {
          console.warn(`[MonthlyReportScheduler][${tenant.key}] Skipped: database not connected`);
          continue;
        }
        await runWithTenant(tenant, () => runForCurrentTenant(tenant.key));
      }
    },
    {
...
module.exports.runForCurrentTenant = runForCurrentTenant;
