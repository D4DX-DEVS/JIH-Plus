 ...
 * Runs at midnight on January 1st of every year (Asia/Kolkata timezone), once
 * per JIH Portal tenant (master + franchises), each against its own database.
 */
const runForCurrentTenant = async (tenantKey) => {
  const now = new Date();
  const currentYear = now.getFullYear();
  const tag = `[YearlyReportScheduler][${tenantKey}]`;

  console.log(`${tag} Running job for year ${currentYear}`);
  ...
  try {
    ...
    for (const template of templates) {
      ...
      console.log(
        `${tag} Created yearly report ${newReport._id} from template ${templateRootId} for year ${currentYear}`
      );
    }
  } catch (error) {
    console.error(`${tag} Error generating yearly reports:`, error);
  }
};
  ...
  cron.schedule(
    ...
    async () => {
      for (const tenant of listTenants()) {
        if (!tenant.isReady()) {
          console.warn(`[YearlyReportScheduler][${tenant.key}] Skipped: database not connected`);
          continue;
        }
